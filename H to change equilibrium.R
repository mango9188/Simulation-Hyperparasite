...
f_outcome = function(state, parms){
  pop_size = ode(func = M2,
                 ...
                 parms = parms)
  Outcome =
    paste(ifelse(pop_size[length(times), c("H", "P1H", "P2H", "P1", "P2")] > 1e-7, "T", "F"), collapse = "")
  return(Outcome)
}

#function of bisection method
Bisection_H = function(min_H, max_H, parms){
  ###The goal is to find a maximum value of H that make competition outcome == E2H
  while(max_H - min_H < 1e-3){
    ###Min
    temp_state["H"] = min_H
    min_E = f_outcome(temp_state, parms)
    
    ###Max
    temp_state["H"] = max_H
    max_E = f_outcome(temp_state, parms)
    
    ###Mid
    mid_H = (min_H + max_H)/2
    temp_state["H"] = mid_H
    mid_E = f_outcome(temp_state, parms)
...
for (i in 1:dim(P.space)[1]) {
  ...
  P.space[i,]$H_press = Bisection_H(min(P.space[i, "H"]))
}

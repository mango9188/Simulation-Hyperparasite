...
ini_Low_H <- c(H = 0.01, P1H = 0, P2H = 0, P1 = 0.01, P2 = 0.01, S = 0.5)
parms_EC_E2H =
  c(r = 1, K = 10,
...
#Set up
Low_H_EC_E2H$H_press = 0

#Run a for-loop
Start_time = Sys.time()
for (i in Invasion_timing) {
  #setting the initial condition
  #temp_state = round(unlist(Low_H_EC_E2H[i,  c("H", "P1H", "P2H", "P1", "P2", "S")]), 8) #use unlist() to make values become vectors
  temp_state = round(unlist(Low_H_EC_E2H[round(Low_H_EC_E2H$time, 2) == round(i, 2),  c("H", "P1H", "P2H", "P1", "P2", "S")]), 8)
  
  #run with the bisection method
  Low_H_EC_E2H[i,]$H_press = 
    Bisection_H(min_H = Low_H_EC_E2H[i, "H"],
                max_H = 25,
                state = temp_state,
                parms = parms_EC_E2H)
...
#saveRDS(Low_H_EC_E2H, "Low_H_EC_E2H_invadeTest1")

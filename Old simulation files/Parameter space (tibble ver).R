...
#### Create data frame----
comp_out <- as_tibble(cbind(comp_out,
                                matrix(0, 
                                       nrow = dim(comp_out)[1],
                                       ###dim(data)[1] is the number of row of data; [2] is col
                                       ncol = 6 + 2)))
names(comp_out) <- c("m1", "m2", "EC", "E1H", "E2H", "E1", "E2", "ES", "Stability", "Stable_E")

comp_out = comp_out %>%
  mutate(across(all_of(c("EC", "E1H", "E2H", "E1", "E2", "ES")), as.list))
...
for(i in 1:dim(comp_out)[1]){
  temp_parms = parms
  temp_parms$m1 = comp_out$m1[[i]]
  temp_parms$m2 = comp_out$m2[[i]]

...
#Set up
Low_H_E1H_E2H$H_press = 0

#Run a for-loop
Start_time = Sys.time()
for (i in Invasion_timing) {
  #setting the initial condition
  #temp_state = round(unlist(Low_H_E1H_E2H[i,  c("H", "P1H", "P2H", "P1", "P2", "S")]), 8) #use unlist() to make values become vectors
  temp_state = round(unlist(Low_H_E1H_E2H[round(Low_H_E1H_E2H$time, 2) == round(i, 2),  c("H", "P1H", "P2H", "P1", "P2", "S")]), 8)
  
  #run with the bisection method
  Low_H_E1H_E2H[i,]$H_press = 
    Bisection_H(min_H = Low_H_E1H_E2H[i, "H"],
                max_H = 25,
                state = temp_state,
                parms = parms_E1H_E2H)
...
#saveRDS(Low_H_E1H_E2H, "Low_H_E1H_E2H_invadeTest1")
##Plotting----
#Low_H_E1H_E2H = readRDS("Low_H_E1H_E2H_invade")
Low_H_E1H_E2H %>%
  filter(H_press != 0) %>%
  ggplot()+
  ...
  scale_x_log10()
ggsave("Low_H_E1H_E2H_invasion.png", width = 14, height = 10, units = "cm", dpi = 800)

#High_H_EC_E2H = readRDS("High_H_EC_E2H_invade")
High_H_EC_E2H %>%
  filter(H_press != 0) %>%
  ggplot()+
  geom_line(mapping = aes(x = time, y = H_press-H))+
